...
<livewire:media.index :key="'media-library-component' . $name" :media="$media" :model="$model" :name="$name" :rules="$rules"
    :maxItems="$maxItems" :multiple="$multiple" :sortable="$sortable" :view="$componentView ?? null" :listView="$listView" :itemView="$itemView"
    :propertiesView="$propertiesView ?? null" :fieldsView="$fieldsView ?? null" :editableName="$editableName" />
