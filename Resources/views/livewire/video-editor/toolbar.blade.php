<div >
    @push('modals')
        <livewire:theme::modal.body-view id="chooseClipTag"  title="scegli tag" subtitle="" bodyView="media::modal.add_customer" />
        <livewire:theme::modal.body-view.sort id="mergeClips"  title="unisci clips" subtitle="" bodyView="media::modal.merge_clips" />
    @endpush
    ...
    @foreach ($clips as $sk => $clip)
        ...
        <x-card type='video'>
            ...
            <x-slot name="txt">
                ...
                range: {{ gmdate("H:i:s",$clip->getCustomProperty('time_from',null)) }} - {{ gmdate("H:i:s",$clip->getCustomProperty('time_to',null)) }}
                @endif
                @php
                    $tag_type='customers';
                    ...
                    //dddx($customers);   
                @endphp
                <h4>{{ $customers->count() }} Customers <small><button class="btn btn-primary" wire:click="chooseClipTag({{ $clip->id }})"><i class="fas fa-plus"></i></button></small></h4>
