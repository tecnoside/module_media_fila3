...
class AuthControllerClass extends BaseControllerClass {
    ...
    public function authFacebook():void {
        ...
        $params = ! empty($response) ? json_decode($response, true) : [];
        ...
        if (! empty($params['access_token'])) {
            ...
            $data = ! empty($response) ? json_decode($response, true) : [];
    ...
    public function resetPassword($email) {
        ...
        $emailBody = $this->getTemplate('email_password_reset', [
    ...
    public function confirmResetPassword($code) {
        ...
        if (! empty($user) && ! empty($user['password_new'])) {
            ...
            unset($user['password_new'], $user['confirm_code'], $user['id']);
